...
- `https://brew.family/*`：GMGN/DeBot 内的 Brew 浮窗打开时，只读取官方公开的 `/launch-checkpoint.json` 发行快照，并按发行记录中的完整代币地址和完整官方池地址精确匹配底池；该权限只用于跨站读取公开 JSON，不在 Brew 官网注入内容，也不读取 Brew 钱包、登录态或交易数据。
