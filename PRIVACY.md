...
「参与 FOMO 榜单更新」默认关闭，并在设置中标注 NEW。用户明确开启、已连接 985monitor、持有有效本地 FOMO 登录态且打开 GMGN / DeBot 页面后，才向现有 SSE 通道声明愿意参与。服务器正常每小时从这些在线账号中随机选一名，同账号的多个浏览器会话不增加账号被选中的概率；没有合格用户则等待，不让所有用户轮询榜单。任务绑定该 985monitor 会话，三分钟过期。
...
浏览器本地仅另存参与开关、最近任务 ID、完成状态及时间；服务器保存任务状态（使用散列会话标识）和最近一次最多 100 名的公开数据缓存，公开页沿用前 30 名。输入经过完整性、类型、大小、排名顺序和字段白名单校验，贡献来源标为 opt-in-browser；这不是对 FOMO 原始响应的密码学认证。联盟 / 共同关注不由本功能采集，继续保留旧值及原时间戳。

关闭开关后停止参与，并中止正在进行的采集；服务器可能收到不含榜单的取消状态。错误或超时不发布部分结果、不清空旧数据，不立即更换账号 / 出口重试。接口失败至少退避 12 → 24 → 48 小时，并尊重更长 Retry-After。已发布的公开榜单不会因某位贡献者关闭开关而删除。
