...
better gmgn 只在用户访问 `gmgn.ai`、`debot.ai`、`brew.family`、`fomo.family` 与 `985monitor.xyz` 时注入固定的本地扩展代码，用于页面增强、公开发行/底池展示、登录态镜像和配置同步；扩展不包含远程执行代码。

## 本地保存的数据

- 重点 Dev、特别关注、备注、功能开关、颜色、黑名单、标注人物、持仓提醒清单、Brew 公开行情缓存和最近 100 条推送历史保存在浏览器本地 `chrome.storage.local`。Brew 缓存只包含公开的代币、官方池地址与市场数据；提醒历史只包含提醒类型、代币简称、触发值、时间和 GMGN 站内代币路径，不包含钱包地址或账号标识。
...
- `https://prod-api.fomo.family/*`、`https://fomo.family/*`：读取 FOMO 持仓者、观点、交易与当前热门代币数据，并保活一个由站点 Privy SDK 自行续期的真实页面；扩展不直接请求 Privy sessions 接口。热门数据只在用户打开 GMGN 的 `fomo` 热门标签时读取，后台以 15 秒缓存和并发合并减少重复请求。
...
- `https://brew.family/*`：只在 Brew 页面读取官方公开的 `/launch-checkpoint.json` 发行快照，展示新创建代币，并按发行记录中的完整代币地址和完整官方池地址精确匹配底池；不读取 Brew 钱包、登录态或交易数据。
- `https://api.dexscreener.com/latest/dex/pairs/bsc/*`：仅在 Brew 战壕浮窗打开时，按最多 30 个官方池一组读取公开的流动性、24 小时成交额/涨跌与市值数据；2 分钟缓存并合并同一轮请求，不搜索或回退到同名/其他底池。该接口允许跨源公开读取，扩展未为它申请额外主机权限。
