...
- 相似币浮窗的保留结果只放在当前 GMGN / DeBot 标签页内存，最多 400 项，包括代币名称、ticker、链、地址、公开市值和底池字段；不写入磁盘、不上传服务器。默认保留 5 分钟，可配置 1 / 5 / 10 / 30 分钟；关闭功能或刷新页面后清空。保留时长这一设置本身保存在 `chrome.storage.local`。市值来自当前页面原生追踪记录的最新缓存，数值更新不延长结果保留期限；未有新追踪数据时保留最后值。DeBot 的名称/头像/底池资料通过用户浏览器向 DeBot 自己的同源接口读取，不发送到 985monitor；DeBot 长按屏蔽只写插件本地名单，不改 DeBot 账户黑名单。
...
用户逐人启用后，扩展仅保存 GMGN / DeBot 原生追踪流已展示的新推送摘要：钱包地址、备注、代币链接、交易去重标识与接收时间。数据使用浏览器本地扩展存储 `gdhPriorityPushV1:` 前缀，不上传服务器，不额外请求交易接口。两站按来源隔离，同站标签页同步关闭状态。

未关闭的提醒不按时间或条数自动删除；关闭后移除正文，仅保留小型去重标识，防止旧推送再次出现。取消人物勾选或关闭特别关注高亮只隐藏提醒，不删除未关闭记录。此缓存不读取私钥、助记词、登录令牌，不触发交易。
