...
- 985monitor 的 FOMO/Pump 屏蔽名单、事件偏好、关注钱包和金额/代币过滤会从该站点同步到扩展本地，用于 GMGN 与 DeBot 追踪页过滤；用于读取个人 Pump 配置的站点令牌只随同源请求发回 985monitor，不写入扩展存储。
...
- `https://985monitor.xyz/*`：读取公开的 FOMO/Pump 事件流、默认 Pump 关注列表和默认标注人物持仓产物，并在用户已登录 985monitor 时读取其个人 Pump 过滤配置。用户自己添加的标注人物不会自动上报服务器，而是在浏览器内直查 GMGN。
