...
- FOMO 页面使用 Privy 登录。扩展会把页面已有的 access token 与 refresh token 镜像到扩展本地存储；令牌只用于请求 `fomo.family` 自己的 API。续期由真实 FOMO 应用页中的 Privy SDK 完成；调用页面已有的 getAccessToken 方法，扩展不再向网页写回旧会话，扩展不把令牌发给 985monitor 或其他第三方。
...
v0.46.72 移除独立参与开关，v0.46.73 将说明收敛到版本按钮旁的普通小字，不自动开页、不弹窗、不显示醒目提示框。用户同意更新说明后，默认参与公开榜单更新；说明在更新确认中明示。由旧更新器升级或手动安装、没有确认记录的用户，可在版本区点击同一个按钮「同意并使用当前版」确认，确认前不采集，其他功能不受影响；旧版主动开启过参与功能的授权继续有效。授权记录只保存在本地，包含说明版本与确认时间。

插件总开关开启、已授权、已连接 985monitor、持有有效本地 FOMO 登录态且打开 GMGN / DeBot 页面后，才向现有 SSE 通道声明参与。服务器正常每小时从这些在线账号中随机选一名，同账号多个会话不增加被选概率；没有合格用户则等待，不让所有用户轮询榜单。任务绑定该 985monitor 会话，三分钟过期。

被选中后，浏览器向 FOMO 固定官方接口依次读取总榜、30 天、7 天、24 小时收益榜，每榜最多 100 名，沿用原有全局串行队列和限流冷却。传给 985monitor 的仅是榜单中公开的排名、人物 ID、handle、名字、公开 X 字段、头像链接、粉丝数、交易次数、成交量、盈亏；不包含参与者的 FOMO access / refresh token、Cookie、钱包私钥、个人关注名单或私人交易历史。上传使用已有 985monitor 会话鉴权，但该会话仅能交回服务器分配的有效任务，不能任意覆盖榜单。

浏览器本地仅另存更新说明确认版本与时间、最近任务 ID、完成状态及时间；服务器保存任务状态（使用散列会话标识）和最近一次最多 100 名的公开数据缓存，公开页沿用前 30 名。输入经过完整性、类型、大小、排名顺序和字段白名单校验，贡献来源标为 opt-in-browser；这不是对 FOMO 原始响应的密码学认证。联盟 / 共同关注不由本功能采集，继续保留旧值及原时间戳。

关闭插件总开关后停止参与，并中止正在进行的采集；服务器可能收到不含榜单的取消状态。错误或超时不发布部分结果、不清空旧数据，不立即更换账号 / 出口重试。接口失败至少退避 12 → 24 → 48 小时，并尊重更长 Retry-After。已发布的公开榜单不会因某位贡献者停止参与而删除。
...
## 985 人物钱包到 GMGN 的手动联动（v0.46.113）

- 在 `985monitor.xyz`、`www.985monitor.xyz`、`985.nz`、`www.985.nz` 的人物详情内显示添加按钮。未安装插件时没有此按钮。
- 仅用户真实点击时，把所选链、该公开钱包地址和人物名交给同浏览环境的 GMGN 页面；正常与隐身环境不混用。没有打开 GMGN 时只打开页面，登录后不自动执行关注。
- GMGN 访问令牌在 GMGN 页内读取、只发回 GMGN，不经 985 服务器、不写入 985 存储，也不随扩展回复返回。只回传固定成功或失败状态。
- 先读取 GMGN 当前链的关注名单；已关注时不覆盖备注、分组；无法确认完整名单时停止。确认未关注后才提交添加与人物名备注。不涉及交易、钱包授权、签名或转账。
- 本功能不新增后台定时请求、不收集用户的整个 GMGN 名单、不把名单持久化或上传，不接受外部网页消息触发写入。
