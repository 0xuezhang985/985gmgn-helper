...
try {
  for (const site of ['gmgn', 'debot']) {
    ...
    assert.equal(await page.evaluate(() => stored.priorityBuyStrategies.group.windowSeconds), 120);
    assert.equal(await page.evaluate(() => stored.priorityBuyStrategies.group.enabled), true);
    pass(`${site}: 500 次扫描、人物名单变化及切换分栏不丢草稿，保存原配置成功`);
    await field('group-wallets').fill(A); await page.getByRole('button', { name: '保存策略', exact: true }).click();
    ...
    await page.evaluate(() => { failWrite = false; }); await page.getByRole('button', { name: '保存策略', exact: true }).click();
    await page.waitForFunction(() => stored.priorityBuyStrategies.amount.minUsd === 2300.5);
    pass(`${site}: 无效配置不保存，存储失败保留草稿并可重试`);
    await page.evaluate(() => { stored.priorityBuyStrategies.group.windowSeconds = 333; settings.priorityBuyStrategies = structuredClone(stored.priorityBuyStrategies); scan(); });
    assert.equal(await field('group-window').inputValue(), '333');
    await field('group-window').fill('444');
    await page.evaluate(() => { stored.priorityBuyStrategies.group.windowSeconds = 555; settings.priorityBuyStrategies = structuredClone(stored.priorityBuyStrategies); scan(); });
    assert.equal(await field('group-window').inputValue(), '444');
    assert.equal(await page.getByRole('button', { name: '保存策略', exact: true }).isDisabled(), true);
    await page.getByRole('button', { name: '重新读取' }).click();
    ...
    await page.evaluate(() => { stored.priorityBuyStrategies = structuredClone(stored.priorityBuyStrategies); stored.priorityBuyStrategies.group.windowSeconds = 777; });
    await page.getByRole('button', { name: '保存策略', exact: true }).click();
    await page.waitForFunction(() => document.querySelector('.gdh-strategy-status').textContent.includes('其他页面修改'));
    assert.equal(await page.evaluate(() => stored.priorityBuyStrategies.group.windowSeconds), 777);
